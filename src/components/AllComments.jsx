            ...
            <tbody>
              {comments.map((comment, index) => {
                return (
                  <tr key={comment._id}>
                    <th>{index + 1}</th>
                    <td className="comment-author">{comment.author}</td>
                    <td className="comment-content">{comment.content}</td>
                    <td>
                      <Link
                        to={`/dashboard/posts/${comment.post}/comments/${comment._id}/edit`}
                      >
                    ...
                    <td>
                      <Link
                        to={`/dashboard/posts/${comment.post}/comments/${comment._id}/delete`}
                      >
